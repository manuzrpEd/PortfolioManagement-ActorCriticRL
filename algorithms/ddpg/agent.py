...
class CriticNetwork(nn.Module):
    ...
    def save_checkpoint(self, round, iteration, address=None):
        if address is None:
            address = self.checkpoint_dir
        T.save(self.state_dict(), f'{address}/{self.name}_{round}_{iteration}')

    def load_checkpoint(self, round, iteration, address=None):
        if address is None:
            address = self.checkpoint_dir
...
class Agent(object):
    ...
    def save_models(self, round, iteration, address):
        ...
        self.target_actor.save_checkpoint(round, iteration, address)
        self.critic.save_checkpoint(round, iteration, address)
        self.target_critic.save_checkpoint(round, iteration, address)

    def load_models(self, round, iteration, address):
        print('... loading models ...')
        self.actor.load_checkpoint(round, iteration, address)
        self.target_actor.load_checkpoint(round, iteration, address)
        self.critic.load_checkpoint(round, iteration, address)
        self.target_critic.load_checkpoint(round, iteration, address)
